...
public class BeaconConnection {
	...
	private boolean mStartWritePass = false;// Jaalee
	public boolean mCurrentIsJaaleeNewBeacon = false;// Jaalee
	...
	public BeaconConnection(Context context, Beacon beacon,
		...
		this.uuidToService
				.put(JaaleeUuid.JAALEE_BEACON_SERVICE, mBeaconService);
		this.uuidToService.put(JaaleeUuid.BEACON_ALERT, mAlertService);
		this.uuidToService.put(JaaleeUuid.BEACON_NAME, mNameService);
		this.uuidToService.put(JaaleeUuid.BEACON_STATE_SERVICE,
				mBeaconStateService);
		this.uuidToService.put(JaaleeUuid.BEACON_AUDIO_STATE_SERVICE,
				mBeaconAudioStateService);
		this.uuidToService.put(JaaleeUuid.BEACON_TX_POWER_SERVICE,
				mBeaconTxPowerService);
	}

	public BeaconConnection(Context context, BLEDevice beacon,
			ConnectionCallback connectionCallback) {
		mCurrentIsJaaleeNewBeacon = false;
		this.context = context;
		this.device = deviceFromBeacon(beacon);
	...
	public boolean StartNotifyMotion() {
		if ((!isConnected())
				|| (!this.mAlertService
						.hasCharacteristic(JaaleeUuid.BEACON_ALERT_CHAR))) {
		...
		for (BluetoothGattService service : this.bluetoothGatt.getServices()) {
			if (UUID.fromString("0000aa10-0000-1000-8000-00805f9b34fb").equals(
					service.getUuid())) {

				BluetoothGattCharacteristic Characteristic = service
						.getCharacteristic(UUID
								.fromString("0000aa12-0000-1000-8000-00805f9b34fb"));

				if (this.bluetoothGatt.setCharacteristicNotification(
						Characteristic, true)) {
					java.util.UUID DescriptorBaseUUID = UUID
							.fromString("00002902-0000-1000-8000-00805f9b34fb");
					BluetoothGattDescriptor descriptor = Characteristic
							.getDescriptor(DescriptorBaseUUID);

					descriptor
							.setValue(BluetoothGattDescriptor.ENABLE_NOTIFICATION_VALUE);

					if (this.bluetoothGatt.writeDescriptor(descriptor)) {
						Log.i("JAALEE", "JAALEE START NOTIFATION SUCCESSFULLY");
						return true;
					} else {
						Log.i("JAALEE", "JAALEE START NOTIFATION FAILED");
						return false;
					}
				} else {
					Log.i("JAALEE", "JAALEE FAILED TO SET NOTIFICATON");
					return false;
				}
			}
		}
	...
	public void CallBeacon() {
		...
		BluetoothGattCharacteristic AlertChar = this.mAlertService
				.beforeCharacteristicWrite(JaaleeUuid.BEACON_ALERT_CHAR, null);
	...
	private void BeaconKeepConnect(WriteCallback writeCallback) {
		if ((!isConnected())
				|| (!this.mBeaconService
						.hasCharacteristic(JaaleeUuid.BEACON_KEEP_CONNECT_CHAR))) {
			L.w("Not connected to beacon. Discarding changing proximity UUID.");
			writeCallback.onError();
			return;
		}
		BluetoothGattCharacteristic uuidChar = this.mBeaconService
				.beforeCharacteristicWrite(JaaleeUuid.BEACON_KEEP_CONNECT_CHAR,
						writeCallback);
	...
	public void ChangePassword(String NewPsaaword, WriteCallback writeCallback) {
		if ((!isConnected())
				|| (!this.mBeaconService
						.hasCharacteristic(JaaleeUuid.BEACON_UUID_CHAR))) {
			L.w("Not connected to beacon. Discarding changing proximity UUID.");
			writeCallback.onError();
			return;
		}
		byte[] OldPassAsBytes = HashCode.fromString(
				mPassword.replaceAll("-", "").toLowerCase()).asBytes();
		byte[] NewPasswordBytes = HashCode
				.fromString(NewPsaaword.toLowerCase()).asBytes();

		final BluetoothGattCharacteristic PasswordChar;

		if (mCurrentIsJaaleeNewBeacon) {
			PasswordChar = this.mBeaconService.beforeCharacteristicWrite(
					JaaleeUuid.BEACON_FFF8, writeCallback);
			PasswordChar.setValue(NewPasswordBytes);
		} else {
			PasswordChar = this.mBeaconService.beforeCharacteristicWrite(
					JaaleeUuid.BEACON_FFF6, writeCallback);
			PasswordChar.setValue(this.arraycat(OldPassAsBytes,
					NewPasswordBytes));
		}
		...
		if (!mCurrentIsJaaleeNewBeacon) {
			new Handler().postDelayed(new Runnable() {
				public void run() {
					...
					BeaconConnection.this.bluetoothGatt
							.readCharacteristic(PasswordChar);
				}
			}, 3000);
		}
	}
	...
	public void writeProximityUuid(String proximityUuid,
		...
		byte[] uuidAsBytes = HashCode.fromString(
				proximityUuid.replaceAll("-", "").toLowerCase()).asBytes();
		byte[] PasswordBytes = HashCode.fromString(mPassword.toLowerCase())
				.asBytes();

		final BluetoothGattCharacteristic uuidChar = this.mBeaconService
				.beforeCharacteristicWrite(JaaleeUuid.BEACON_UUID_CHAR,
						writeCallback);
	 ...
	 *            Advertising interval in milliseconds (100ms-10000ms).
	 * @param writeCallback
	 *            Callback to be invoked when write is completed.
	 */
	public void writeAdvertisingInterval(int BroadcastInterval,
			WriteCallback writeCallback) {
		...
		if ((!isConnected())
				|| (!this.mBeaconService.hasCharacteristic(temp_UUID))) {
			L.w("Not connected to beacon. Discarding changing advertising interval.");
			writeCallback.onError();
			return;
		}

		final BluetoothGattCharacteristic intervalChar = this.mBeaconService
				.beforeCharacteristicWrite(temp_UUID, writeCallback);
		...
		if (mCurrentIsJaaleeNewBeacon) {
			...
			new Handler().postDelayed(new Runnable() {
				public void run() {
					BeaconConnection.this.bluetoothGatt
							.readCharacteristic(intervalChar);
				}
			}, 3000);
		}
	}
	 ...
	 * 
	 * @param //powerDBM
	 *            Beacon's measured power to be set.
	 * @param writeCallback
	 *            Callback to be invoked when write is completed.
	 */
	public void writeBroadcastingPowerValue(int power,
			WriteCallback writeCallback) {
		if ((!isConnected())
				|| (!this.mBeaconService
						.hasCharacteristic(JaaleeUuid.POWER_CHAR))) {
		...
		final BluetoothGattCharacteristic powerChar = this.mBeaconService
				.beforeCharacteristicWrite(JaaleeUuid.POWER_CHAR, writeCallback);
	...
	/**
	 * Changes major value of the beacon. Major value will be normalized to fit
	 * the range (1, 65535).
	 * 
	 * @param major
	 *            Major value to be set.
	 * @param writeCallback
	 *            Callback to be invoked when write is completed.
	 */
	public void writeMajor(int major, WriteCallback writeCallback) {
		...
		final BluetoothGattCharacteristic majorChar = this.mBeaconService
				.beforeCharacteristicWrite(JaaleeUuid.MAJOR_CHAR, writeCallback);
	...
	/**
	 * Changes minor value of the beacon. Minor value will be normalized to fit
	 * the range (1, 65535).
	 * 
	 * @param minor
	 *            Minor value to be set.
	 * @param writeCallback
	 *            Callback to be invoked when write is completed.
	 */
	public void writeMinor(int minor, WriteCallback writeCallback) {
		...
		final BluetoothGattCharacteristic minorChar = this.mBeaconService
				.beforeCharacteristicWrite(JaaleeUuid.MINOR_CHAR, writeCallback);
	...
	public void writeBeaconState(int state, WriteCallback writeCallback) {
		...
		final BluetoothGattCharacteristic Char = this.mBeaconStateService
				.beforeCharacteristicWrite(JaaleeUuid.BEACON_STATE_CHAR,
						writeCallback);
	...
	public void writeBeaconAudioState(int state, WriteCallback writeCallback) {
		...
		final BluetoothGattCharacteristic Char = this.mBeaconAudioStateService
				.beforeCharacteristicWrite(JaaleeUuid.BEACON_AUDIO_STATE_CHAR,
						writeCallback);
		Char.setValue(arrayOfByte);
		this.bluetoothGatt.writeCharacteristic(Char);
	}
	...
	/**
	 * Changes broadcasting power of the beacon. Allowed values: see
	 * JaaleeDefine
	 * 
	 * @param state
	 *            The Tx Power of the beacon
	 * @param writeCallback
	 *            Callback to be invoked when write is completed.
	 */
	public void writeBeaconTxPower(int state, WriteCallback writeCallback) {
		...
		final BluetoothGattCharacteristic Char = this.mBeaconTxPowerService
				.beforeCharacteristicWrite(JaaleeUuid.BEACON_TX_POWER_CHAR,
						writeCallback);
	...
	public void writeBeaconName(String name, WriteCallback writeCallback) {
		...
		if (mCurrentIsJaaleeNewBeacon) {
			Char = this.mNameService.beforeCharacteristicWrite(
					JaaleeUuid.BEACON_NEW_NAME_CHAR, writeCallback);
		} else {
			Char = this.mNameService.beforeCharacteristicWrite(
					JaaleeUuid.BEACON_NAME_CHAR, writeCallback);
		}
	...
	private BluetoothGattCallback createBluetoothGattCallback() {
		return new BluetoothGattCallback() {
			public void onConnectionStateChange(BluetoothGatt gatt, int status,
				...
				if (newState == 2) {
					L.d("Connected to GATT server, discovering services: "
						...
						&& (!BeaconConnection.this.didReadCharacteristics)) {
			...
			public void onCharacteristicChanged(BluetoothGatt gatt,
				...
				// Log.i("JAALEE", "JAALEE JAALEE UPDATE BATTERY");
				Log.i("JAALEE", "JAALEE NOTIFICATION SUCCESS");
			}
			...
			public void onCharacteristicRead(BluetoothGatt gatt,
				...
				if (status == 0) {
					...
					if (JaaleeUuid.BEACON_KEEP_CONNECT_CHAR
						...
						if (!mPassWordWriteSuccess) {
							BeaconConnection.this.handler.postDelayed(
									new Runnable() {
										@Override
										public void run() {
											// TODO Auto-generated method stub
											BeaconConnection.this
					...
					BluetoothService temp = ((BluetoothService) BeaconConnection.this.uuidToService
			...
			public void onCharacteristicWrite(BluetoothGatt gatt,
				...
				if (JaaleeUuid.BEACON_KEEP_CONNECT_CHAR.equals(characteristic
						.getUuid())) {
			...
			public void onServicesDiscovered(BluetoothGatt gatt, int status) {
				if (status == 0) {
					...
					BeaconConnection.this.processDiscoveredServices(gatt
							.getServices());
					// BeaconConnection.this.startAuthentication(gatt);

					for (BluetoothGattService service : gatt.getServices()) {
						if (JaaleeUuid.BEACON_STATE_SERVICE.equals(service
								.getUuid())) {
						...
						} else if (JaaleeUuid.JAALEE_BEACON_SERVICE
							...
							BeaconConnection.this.handler.postDelayed(
									new Runnable() {
										...
										public void run() {
											// TODO Auto-generated method stub
											BluetoothGattCharacteristic keepChar = BeaconConnection.this.mBeaconService
													.getKeepUUIDChar();
											BeaconConnection.this.bluetoothGatt
...
//		this.toFetch.addAll(this.mBeaconAudioStateService.getAvailableCharacteristics());
	...
	private void onAuthenticated() {
		...
		// this.handler.removeCallbacks(this.timeoutHandler);
		this.didReadCharacteristics = true;
		this.connectionCallback.onAuthenticated(null);
//		this.connectionCallback.onAuthenticated(new BeaconCharacteristics(
//				this.mBeaconService, this.mNameService,
//				this.mBeaconTxPowerService, this.mBeaconStateService,
//				this.mBeaconAudioStateService));
	}
